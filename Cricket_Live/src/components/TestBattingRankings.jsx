...
const TestBattingRankings = () => {
  const { isPending, error, data } = useQuery({
    queryKey: ["testBattingRankings"],
    queryFn: () => getTestBattingRankings(),
    staleTime: 120 * 60 * 1000, // Applying caching for 2 hours
  });

  const getTestBattingRankings = async () => {
    const options = {
      method: "GET",
...
export default TestBattingRankings;
